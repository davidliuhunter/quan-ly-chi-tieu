...
function FormFields({ editing, filteredCategories, onFilterChange, filter }: { editing: Transaction | null; filteredCategories: Category[]; onFilterChange: (v: "income" | "expense") => void; filter: string; }) {
  ...
  return (<>
    <div className="space-y-4">
      ...
      <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Số tiền (VNĐ)</label><input name="amount" type="number" inputMode="decimal" min={1000} step={1000} defaultValue={editing?.amount ?? ""} className="input-field" placeholder="VD: 50000" required /><div className="flex flex-wrap gap-2 mt-2">{QUICK_AMOUNTS.map((amt) => (<button key={amt} type="button" onClick={(e) => { const form = (e.target as HTMLElement).closest("form"); const inp = form?.querySelector<HTMLInputElement>("input[name=\"amount\"]"); if (inp) { inp.value = String(amt); inp.dispatchEvent(new Event("input", { bubbles: true })); } }} className="px-3 py-1.5 text-xs rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-blue-100 dark:hover:bg-blue-900/30 hover:text-blue-600 dark:hover:text-blue-400 active:scale-95 transition-all">{amt.toLocaleString("vi-VN")}</button>))}</div></div>
